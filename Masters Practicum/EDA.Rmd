...
```{r}
claims %>% select(ED_DISCHARGE_DX_DESC, CODE_1,CODE_2, ED_NOT_NEEDED_PROP, PREVENTABILITY) %>% group_by(ED_DISCHARGE_DX_DESC, CODE_1,CODE_2) %>% summarize(ED_NOT_NEEDED_PROP = mean(ED_NOT_NEEDED_PROP), PREVENTABILITY = mean(PREVENTABILITY)) %>% View()
```

...
claims <- claimsFull %>%
  select(MRN_ALIAS, MEMBER_SEX,MEMBER_AGE,CLAIM_SEQ,EPISODE_SEQ,APPROVED_AMT,
         APPROVED_DAYS,ED_DISCHARGE_DX_DESC,PREVENTABILITY,ED_NOT_NEEDED_PROP,
...
claims %>%
  select(MRN_ALIAS, CLAIM_SEQ, EPISODE_SEQ,ED_DISCHARGE_DX_DESC, PREVENTABILITY,ED_NOT_NEEDED_PROP,
         UNCLASSIFIED_ED, SERVICE_TYPE,CLAIM_TYPE,PLACE_OF_SERVICE_DESC, TOB_CATEGORY) %>%
  filter(MRN_ALIAS == '98D3D44BDEB2',SERVICE_TYPE == 'ED' & (ED_NOT_NEEDED_PROP == 1 | PREVENTABILITY == 1)) %>%
  arrange(MRN_ALIAS, CLAIM_SEQ, EPISODE_SEQ) 
```

...
UNCLASSIFIED_ED = mean(UNCLASSIFIED_ED),NumberOfClaims = n())
...
#remove rows where ED_DISCHARE_DX_DESC as this indicates a invalid code, usualy missing last digit.
icd9Only <- diag %>% 
...
Now that I have ICD-9 codes, I want to look at how many parent codes there are in our dataset.  I want to see the

```{r}
diag %>% group_by(CODE_P) %>% 
  summarize(DistinctCodes=n(), ED_NOT_NEEDED_PROP = mean(ED_NOT_NEEDED_PROP),
...
Read in ICD9 to ICD10 crosswalk.  The issue is there are many ICD10s to few ICD9s thus ICD9s can been be converted into serveral differnt ICD10 flavors.  Using Parent code and keeping ICD9s may be a better option.  Either way, its here to use.  Some lines are not a 1 to 1 and get discarded.

```{r}
cx <- read_delim("Cm9To10.txt", delim = "\t", col_names = FALSE)
cx <- separate(cx, X1, c('ICD9','ICD10'), remove = TRUE)
```

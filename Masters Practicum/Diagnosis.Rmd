...
APPROVED_DAYS,starts_with("CODE_"),  ED_DISCHARGE_DX_DESC,PREVENTABILITY,ED_NOT_NEEDED_PROP,
...
```{r}
#this code gives you a dataframe of all 18 codes in two columns code and frequency
...
The section is used to diagnosis duplicates and mess code formats to make sure the joins don't introduce any duplication
...
n_occur1 <- data.frame(table(diag$CODE_FULL))

n_occur2 <- data.frame(table(diag2$CODE_FULL))

n_occur3 <- data.frame(table(diag3$CODE_FULL))

n_occur4 <- data.frame(table(diag_full$CODE_FULL))

n_occurcx <- data.frame(table(cx$CODE_FULL))

n_occurcci <- data.frame(table(cci_both$CODE))

n_occurfinal <- data.frame(table(code_final$CODE_FULL))
```
...
Import Virginia Premier Crosswalk
...
code_final <- diag_full %>% 
  left_join(cci_both, by = c('CODE_FULL' = 'CODE', 'ICD_VERSION' = 'ICD_VERSION')) %>% 
...
names(code_final)[names(code_final) == 'CODE_FULL'] <- 'DIAG_CODE'


# join crosswalk
code_final2 <- code_final %>% 
  left_join(cx, by = c('DIAG_CODE' = 'DIAG_CODE', 'ICD_VERSION' = 'ICD_VERSION'))

code_final2 <- code_final2 %>% 
  select(DIAG_CODE,ICD_VERSION,DIAG_CODE_DESC,CCS_CATEGORY, CCS_CATEGORY_DESC,
         CHRONIC_INDICATOR, BODY_SYSTEM, BODY_SYSTEM_DESC, everything(), -CODE_DESCRIPTION)

glimpse(code_final2)
summary(code_final2)

write.csv(code_final2, 'Data/diagnosis_final.csv')
```
